...
class WithElements extends WithDb
{
	...
	if (!isset($this->division)) {
	    ...
	    if (isset($arraylist) && is_array($arraylist) && $arraylist !==null) {
			...
			if (isset($this->alias)) {
				...
				//$beforealias = "/" . $this->division . "/" .$arrayalias[$minid];
				//$beforetitle = $arraytitle[$minid];
				}
		} 
	...
	/* not free
	 * Получаем данные с формы, обрабатываем их и отправляем на модерацию*/
	public function actionAddComment()
	{
        ...
        $this->numberuser;
        $this->moder;
        if (isset($this->nicknameuser))
        if (isset($this->comment)) {
			/* Получаем номер статьи*/
			if (isset($this->sectionin)) {
			if (isset($this->department) && $this->department === $this->division)
			$namearticle =$this->sectionin;
		    } else {
				if (isset($this->department))
				$namearticle =$this->department;
				}
			$this->idar = $namearticle;
			$this->divar = $this->department;
			$this->table = "feedback" . $this->department;
			$this->insertComment();		    
		}
	...
	/* not free*/
	protected function supportNick()
	{
		if (isset($_COOKIE['nicknameuser']))
		$this->nicknameuser = $_COOKIE['nicknameuser'];

		if (isset($this->nicknameuser)) return $this->nicknameuser;
		else return null;
	}//supportNick
	...
	/* protected function supportingLinkCross()
	 {
		 //$dom = $this->viewDomain();
			if (isset($this->alias) && $this->alias !== "") {
				$linkCrossing = "http://apiunatka/login/supportive.php?division="
			...
			if (isset($linkCrossing)) return $linkCrossing;
	  }*///supportingLinkCross()
